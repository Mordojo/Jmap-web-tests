...
${TYPE}    WEB
${tc_start}    ${EMPTY}
${IMAGE_COMPARATOR_COMMAND}   "C:${/}Program Files${/}ImageMagick-7.0.9-Q16${/}convert" __REFERENCE__ __TEST__ -metric RMSE -compare -format  "%[distortion]" info:
${useTL}    true
#-----------------------------------------------------------------------------------------------------------------------------------
*** Keywords ***
Get Formated Current Date And Time
    ${FORMATED_DATE}    Get Current Date    result_format=%Y%m%d%H%M%S
    Set Suite Variable    ${FORMATED_DATE}
...
Go To Testlink To Update Test Case
    ...
    \    Log    ---- > ${getExecution} 
    \    &{dictIfFail}    Create Dictionary    id    -1
    \    @{listIfFail}    Create List    ${dictIfFail}
    \    @{getExecution}    Run Keyword If    '${getExecution[0]}' == 'FAIL'    Create List    PASS    ${listIfFail}    ELSE    Copy List    ${getExecution}        
    ...
    \    ${numVersion}    Set Variable If    ${numVersion} == 0    001       
    \    ...    ${numVersion} > 0 and ${numVersion} < 10    00${numVersion}
    \    ...    ${numVersion} > 10 and ${numVersion} < 100    0${numVersion}
    \    ...    ${numVersion} > 100    ${numVersion} 
    \    Exit For Loop If    '${getExecution[0]}' == 'PASS' and '${idBuild}' != '-1' or '${numVersion}' == '001'                         
...
Get All Ticket Keys Of The Last Execution Notes
    ...
    \    Run Keyword If    '${JiraTicketStatus[2]}' != 'Done' and '${JiraTicketStatus[2]}' != 'Closed' and '${JiraTicketStatus[2]}' != 'Fini' and '${JiraTicketStatus[2]}' != 'Fermée'    Append To List    ${notesNotClosed}    ${Key} ${JiraTicketStatus[1]}                      
